...
class NordicPCA10040(Board):
    ...
    @staticmethod
    def match(dev):
        return dev["vid"] in NordicPCA10040.ids_vendor and dev["pid"] in NordicPCA10040.ids_vendor[dev["vid"]]

    def _exe_jlink_script(self, jlink_script_content):
        jlink_script = fs.get_tempfile(jlink_script_content)
        res,out,err = proc.runcmd("jlinkexe", "-If", "swd", "-device", "nRF52832_xxAA", "-speed", "4000", "-autoconnect", "1", "-CommanderScript", jlink_script)
